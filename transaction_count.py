...
if not os.path.exists(tr_dir):
    os.mkdir(tr_dir)                                    # Клать файлы транзакций в папку Transactions
data_in_path = os.path.join(base_dir, tr_dir)        # Подставить свои пути для файлов транзакций (прямые слэши)

out_dir = 'OutReports'
if not os.path.exists(out_dir):
    os.mkdir(out_dir)
data_out_path = os.path.join(base_dir, out_dir)                # И для выходного файла
